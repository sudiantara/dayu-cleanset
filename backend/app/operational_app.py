...
def operational_control_v1():
    ...
    for row in rows:
        ...
        orders.append({
            ...
            "service_speed":row[5],"status":row[6],"payment_status":row[7],"total_weight":float(row[8] or 0),
            "total":float(row[9] or 0),"requested_finish_at":row[10].isoformat() if row[10] else None,
            "created_at":row[11].isoformat(),"updated_at":row[12].isoformat(),"deadline_state":row[13],
    ...
    handover={
        ...
        "active":len(orders),
        "washing":sum(1 for o in orders if o["status"]=="WASHING"),
        "ready":len(ready),
        "picked_up":sum(1 for o in orders if o["status"]=="PICKED_UP"),
        "delivery":sum(1 for o in orders if o["status"]=="DELIVERING"),
        "overdue":sum(1 for o in orders if o["deadline_state"]=="OVERDUE"),
        "due_soon":sum(1 for o in orders if o["deadline_state"]=="DUE_SOON"),
        "ready_over_2h":sum(1 for o in ready if (o["ready_age_minutes"] or 0)>=120),
        "ready_unpaid":sum(1 for o in ready if o["payment_status"]!="PAID"),
    ...
    return {
        "summary":{
            "active":len(orders),
            "overdue":handover["overdue"],"due_soon":handover["due_soon"],"ready":handover["ready"],
            "unpaid":sum(1 for o in orders if o["payment_status"]!="PAID"),"express":handover["express"],
            "ready_over_2h":handover["ready_over_2h"],"exceptions":handover["exceptions"]
        },
        "handover":handover,
