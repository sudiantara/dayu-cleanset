...
def edit_order_v2(order_number: str, request: EditOrderRequest):
    ...
    promo_discount = subtotal * 0.05 if (
        request.instagram_followed and request.google_reviewed
    ) else 0
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    ...
                    SET
                        ...
                        location_notes = %s,
                        service_speed = %s,
                        requested_finish_at = %s,
                        total_weight = %s,
                        instagram_followed = %s,
                        google_reviewed = %s,
                        promo_discount = %s,
                        special_discount = %s,
                        special_discount_reason = %s,
                        subtotal = %s,
                        discount = %s,
                        total = %s,
                        notes = %s,
                    ...
                    """,
                    (
                        request.hotel_name.strip(),
                        (request.room_number or "").strip() or None,
                        (request.location_notes or "").strip() or None,
                        speed,
                        ...
                        (request.special_discount_reason or "").strip() or None,
                        subtotal,
                        discount,
                        total,
                        (request.notes or "").strip() or None,
                        order_number,
                    ),
                )
                ...
                cursor.execute(
                    ...
                    SELECT id, status, %s, %s
                    FROM orders
                    WHERE order_number = %s
                    """,
                    (
                        f"Data order diedit oleh {user[1]}",
                        request.updated_by,
                        order_number,
                    ),
                )
    ...
    try:
        with get_db_connection() as connection:
            with connection.cursor() as cursor:
                ...
                if status == "COMPLETE" and order[2] != "PAID":
                    raise HTTPException(
                        status_code=409,
                        detail="Order belum lunas dan tidak boleh COMPLETE",
                    )
                ...
                cursor.execute(
                    ...
                    SET
                        ...
                        updated_at = NOW(),
                        completed_at = CASE
                            WHEN %s IN ('COMPLETE', 'PICKED_UP') THEN NOW()
                            ELSE completed_at
                        END
                    WHERE id = %s
                    """,
        ...
        send_n8n_event(
            ...
            message=f"Status {order_number} menjadi {status}",
            data={
                "order_number": order_number,
                "old_status": order[1],
                "status": status,
        ...
        with get_db_connection() as connection:
            with connection.cursor() as cursor:
                ...
                if user[2].upper() != "ADMIN":
                    raise HTTPException(
                        status_code=403,
                        detail="Hanya ADMIN yang boleh menghapus order",
                    )
                ...
                if float(order[2]) > 0:
                    raise HTTPException(
                        status_code=409,
                        detail="Order yang sudah memiliki pembayaran tidak boleh dihapus",
                    )
