...
export default function OrderListPage({ onNewOrder }) {
  ...
  useEffect(() => {
    setPage(1);
  }, [query, status, payment, speed, dateFrom, dateTo]);
  ...
  const totals = useMemo(() => {
    ...
    const active = filtered.filter((order) =>
      !["COMPLETE", "PICKED_UP", "CANCELLED"].includes(order.status),
    ).length;
  ...
  return (
    <div className="order-list-page">
      <header className="order-page-header">
        <div>
          <h1>Order</h1>
          <p>Kelola dan cari seluruh order laundry.</p>
        </div>
        <button className="order-new-button" type="button" onClick={onNewOrder}>
          + Order Baru
        </button>
      </header>
      ...
      <section className="order-panel">
        <div className="order-filter-grid">
          <label className="order-search-field">
            ...
            <input
              ...
              placeholder="No. order, customer, HP, hotel, room..."
            />
          </label>
        ...
        {loading ? (
          <div className="order-loading">Memuat semua order...</div>
        ) : (
          <>
            <div className="order-table-wrap">
              <table className="order-full-table">
                <thead>
                  <tr>
                    <th>Order</th>
                    <th>Customer</th>
                ...
                <tbody>
                  {pageRows.map((order) => (
                    <tr key={order.order_number}>
                      <td><OrderDetailTrigger orderNumber={order.order_number} /></td>
                      <td>
                        <strong>{order.customer || "-"}</strong>
                        <small>{order.phone || ""}</small>
                      </td>
                      <td>{order.service_speed || "-"}</td>
                      <td>{Number(order.total_weight || 0)} KG</td>
                  ...
                  {pageRows.length === 0 && (
                    <tr><td colSpan="8" className="order-empty">Tidak ada order yang cocok dengan filter.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className="order-pagination">
              <span>
                ...
                {" - "}
                {Math.min(currentPage * pageSize, filtered.length)} dari {filtered.length} order
              </span>
              <div>
                <button type="button" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>← Sebelumnya</button>
                <strong>{currentPage} / {totalPages}</strong>
                <button type="button" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Berikutnya →</button>
              </div>
            </div>
          </>
        )}
